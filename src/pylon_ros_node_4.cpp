...
int main(int argc, char* argv[]){
    ros::init(argc, argv, "pylon_node_4" );
    ros::NodeHandle pylon;
    ros::Publisher image_pub = pylon.advertise<sensor_msgs::Image>("/pylon/raw_image_4", 10);
    ...
    cv::namedWindow("pylon_image_4",0);
    //cv::resizeWindow("pylon_image_3", 640, 480);

    try {

         Pylon::CDeviceInfo a_camera;
         a_camera.SetSerialNumber("23443168");
	...
	//Pylon::CInstantCamera camera(Pylon::CTlFactory::GetInstance().CreateFirstDevice());
        ...
        while (camera.IsGrabbing()) {
            ...
            header.frame_id = "Pylon_camera_4";
		...
		cv::resize(Image, mini_Image, cv::Size(960, 600));
                ...
                cv::imshow("pylon_image_4", mini_Image);
