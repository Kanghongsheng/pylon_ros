...
int main(int argc, char* argv[]){
    ros::init(argc, argv, "pylon_node_2" );
    ros::NodeHandle pylon;
    ros::Publisher image_pub = pylon.advertise<sensor_msgs::Image>("/pylon/raw_image_2", 10);
    ...
    cv::namedWindow("pylon_image_2",0);
    //cv::resizeWindow("pylon_image_2", 640, 480);

    try {

         Pylon::CDeviceInfo a_camera;
         a_camera.SetSerialNumber("23443194");
	...
	//Pylon::CInstantCamera camera(Pylon::CTlFactory::GetInstance().CreateFirstDevice());
        ...
        while (camera.IsGrabbing()) {
            ...
            header.frame_id = "Pylon_camera_2";
		...
		cv::resize(Image, mini_Image, cv::Size(960, 600));
                ...
                cv::imshow("pylon_image_2", mini_Image);
